...
whose `HEAD` is the reviewed annotated `v1.0.5` tag. The source build has no remote
...
```text
output/candidate/ARC-1.0.5/ARC-1.0.5.dmg
output/candidate/ARC-1.0.5/ARC-1.0.5-source.tar.gz
output/candidate/ARC-1.0.5/ARC_AI_Relay_Chat_Literature.pdf
output/candidate/ARC-1.0.5/ARC-1.0.5-MANIFEST.json
```
...
```sh
make release-dvt \
  DVT_REPORT=/absolute/path/ARC-1.0.5-DVT-REPORT.txt
```
...
assets under `output/release/ARC-1.0.5/`:

```text
ARC-1.0.5.dmg
ARC-1.0.5-source.tar.gz
ARC_AI_Relay_Chat_Literature.pdf
ARC-1.0.5-MANIFEST.json
ARC-1.0.5-DVT-REPORT.txt
