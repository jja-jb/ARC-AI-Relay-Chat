...
whose `HEAD` is the reviewed annotated `v2.3.0` tag. The source build has no remote
...
staging folders. Defaults are `/private/tmp/arc-build-2.3.0` and
`/private/tmp/arc-output-2.3.0`, outside cloud-synchronized checkouts.
...
```text
/private/tmp/arc-output-2.3.0/candidate/ARC-2.3.0/ARC-2.3.0.dmg
/private/tmp/arc-output-2.3.0/candidate/ARC-2.3.0/ARC-2.3.0-source.tar.gz
/private/tmp/arc-output-2.3.0/candidate/ARC-2.3.0/ARC_AI_Relay_Chat_Literature.pdf
/private/tmp/arc-output-2.3.0/candidate/ARC-2.3.0/ARC-2.3.0-MANIFEST.json
```
...
```sh
make release-dvt \
  DVT_REPORT=/absolute/path/ARC-2.3.0-DVT-REPORT.txt
```
...
assets under `/private/tmp/arc-output-2.3.0/release/ARC-2.3.0/`
(or the explicitly overridden OUTPUT directory):

```text
ARC-2.3.0.dmg
ARC-2.3.0-source.tar.gz
ARC_AI_Relay_Chat_Literature.pdf
ARC-2.3.0-MANIFEST.json
ARC-2.3.0-DVT-REPORT.txt
